...
- [x] 写出资金不变量：只分配一次、总分配不超过存款、Arbiter 永不收款。
...
- [x] 实现 Seller 在 `Funded` 状态取消，款项记给 Buyer。
- [x] 实现交付截止后任意账户触发退款，款项固定记给 Buyer。
- [x] 实现审核截止后任意账户触发结算，款项固定记给 Seller。
- [x] 使用 Hardhat 时间控制测试 `deadline - 1`、`deadline`、`deadline + 1`。
- [x] 对每个终止状态测试所有后续业务操作均失败。
...
- [x] 实现 Buyer 在审核期内 `openDispute()`。
- [x] 实现 Arbiter 二选一裁决：全额给 Seller 或全额退 Buyer。
- [x] 实现仲裁截止后任何人可触发默认退款。
- [x] 测试两种裁决、越权裁决、过期裁决、重复裁决和仲裁人失联。
- [x] 断言 Arbiter 在任何路径都没有待提现余额。
...
- [x] 写最小恶意 Seller：收 ETH 时重入 `withdraw()`。
- [x] 写拒收 ETH 的 Seller：提款时主动 revert。
- [x] 验证重入不能多领，转账失败后待提现余额仍可恢复。
- [x] 写强制转入 ETH 的测试，证明额外余额不改变 `depositAmount` 或结算金额。
- [x] 对权限矩阵和全部非法状态跳转做参数化测试，避免重复测试代码。
- [x] 增加 Solidity fuzz/invariant test：总分配不超过存款、终止后结果不变、Arbiter 永不获款。
...
- [x] 新建 `EscrowFactory`，由它创建独立 Escrow 实例并转发 ETH。
- [x] 修正 Factory 成为 `msg.sender` 后的身份问题：Buyer 必须是最终用户而不是 Factory。
- [x] 通过事件索引订单；不要为了链上列表写无界数组遍历。
- [x] 测试 ETH 转发、Buyer 身份、不同订单隔离、创建失败回滚和事件内容。
- [x] 评估 clone / CREATE2 的收益；没有明确 gas 或地址预测需求就不实现。
